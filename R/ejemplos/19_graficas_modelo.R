...
pacman::p_load("viridis", "latex2exp", "tidyverse", "beepr", "BGPhazard")
...
frailties_chain1 %>%
  bind_rows(frailties_chain2, frailties_chain3) %>%
  group_by(ind) %>%
...
omega1_chain1 %>%
  ...
  ggsave(
    "graphs/ergodic_means_omega1.png",
    width = 30,
    height = 15,
    units = "cm"
    )
...
omega2_chain1 %>%
  ...
  ggsave(
    "graphs/ergodic_means_omega2.png",
    width = 30,
    height = 15,
    units = "cm"
    )
...
tib_lambda1 <-
  ...
  bind_rows(as_tibble(t(out2$lambda1)), as_tibble(t(out3$lambda1)))

tib_lambda2 <-
  as_tibble(t(out1$lambda2)) %>%
  bind_rows(as_tibble(t(out2$lambda2)), as_tibble(t(out3$lambda2)))
...
gamma %>%
  ...
  ylim(c(0, 10)) +
  ggsave(
    "graphs/gamma_ergodic_means.png",
    width = 14,
    height = 10,
    units = "cm"
    )
...
theta %>%
  ...
  ylim(c(0, 1)) +
  ggsave(
    "graphs/theta_ergodic_means.png",
    width = 14,
    height = 10,
    units = "cm"
    )
